...
### Run Nimbus in dev mode
Nimbus can be run in dev mode by cloning this repository and following the ["Contributing" section of our README](https://github.com/vercel/nimbus#contribute).

In dev mode you'll get more ouput and access to React/Redux dev-tools in Electron.

Prerequisites and steps are described in the ["Contributing" section of our README](https://github.com/vercel/nimbus#contribute).
Be sure to use the `canary` branch.

### Create a dev config file
Copy your config file `nimbus.json` to the root of your cloned repository. Nimbus, in dev mode, will use this copied config file. That means that you can continue to use your main installation of Nimbus with your day-to-day configuration.
After the first run, Nimbus, in dev mode, will have created a new `plugins` directory in your repository directory.
...
module.exports = {
  ...
  plugins: [],
  localPlugins: ['nimbus-awesome-plugin'],
  ...
}
```

### Running your plugin
To load, your plugin should expose at least one API method. All possible methods are listed [here](https://github.com/vercel/nimbus/blob/canary/app/plugins/extensions.ts).

After launching Nimbus in dev mode, run `yarn run app`, it should log that your plugin has been correcty loaded: `Plugin nimbus-awesome-plugin (0.1.0) loaded.`. Name and version printed are the ones in your plugins `package.json` file.
...
## Recipes
Almost all available API methods can be found on https://nimbus.is.
If there's any missing, let us know or submit a PR to document it!

### Components
You can decorate almost all Nimbus components with a Higher-Order Component (HOC). To understand their architecture, the easiest way is to use React dev-tools to dig in to their hierarchy.

Multiple plugins can decorate the same Nimbus component. Thus, `Component` passed as first argument to your decorator function could possibly not be an original Nimbus component but a HOC of a previous plugin. If you need to retrieve a reference to a real Nimbus component, you can pass down a `onDecorated` handler.
```js
...
**Bonus feature**: if your command ends with `:prefix`, it would mean that you want to use this command with an additional digit to the command. Then Nimbus will create all your commands under the hood. For example, this keymap `'pane:hide:prefix': 'ctrl+shift'` will automatically generate the following:
```
{
...
### Menu
Your plugin can expose a `decorateMenu` function to modify the Nimbus menu template.
...
### Require Electron
Nimbus doesn't provide a reference to electron. However plugins can directly require electron.
...
## Nimbus v2 breaking changes
Nimbus v2 uses `xterm.js` instead of `hterm`. It means that PTY output renders now in a canvas element, not with a hackable DOM structure.
