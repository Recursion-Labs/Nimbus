...
const checkConfig = () => {
  ...
  msg += 'Please launch Nimbus and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = (pattern?: string) => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${
    (pattern && `${pattern}+`) || ''
  }keywords:nimbus-plugin,nimbus-theme&size=250`;
    ...
    commandPromise = lsRemote(query)
      .then((entries) => {
        if (entries.length === 0) {
          ...
          console.error(`${chalk.red('Try')} ${chalk.green('nimbus ls-remote')}`);
          process.exit(1);
        } else {
...
args.command('<default>', 'Launch Nimbus');
...
const main = (argv: string[]) => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal Nimbus that it was spawned from this module
    NIMBUS_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple Nimbus process
      const cmd = `open -b co.zeit.nimbus ${args_}`;
      const opts = {
        env
      };
