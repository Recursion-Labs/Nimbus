...
import type {NimbusState, NimbusDispatch, NimbusActions, ITermGroups} from '../../typings/nimbus';
import rpc from '../rpc';
...
export function resetFontSize(): NimbusActions {
  return {
...
export function windowGeometryUpdated({isMaximized}: {isMaximized: boolean}): NimbusActions {
  return {
...
const getGroupUids = (state: NimbusState) => {
...
export function enterFullScreen(): NimbusActions {
  return {
    type: UI_ENTER_FULLSCREEN
  };
}

export function leaveFullScreen(): NimbusActions {
  return {
...
export function execCommand(command: string, fn: (e: any, dispatch: NimbusDispatch) => void, e: any) {
  return (dispatch: NimbusDispatch) =>
    dispatch({
