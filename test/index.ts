...
test.before(async () => {
  ...
  switch (process.platform) {
    case 'linux':
      pathToBinary = path.join(__dirname, '../dist/linux-unpacked/nimbus');
      break;

    case 'darwin':
      pathToBinary = path.join(__dirname, '../dist/mac/Nimbus.app/Contents/MacOS/Nimbus');
      break;

    case 'win32':
      pathToBinary = path.join(__dirname, '../dist/win-unpacked/Nimbus.exe');
      break;
